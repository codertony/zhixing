...
import { pgTable, uuid, text, integer, timestamp, json, varchar, boolean } from 'drizzle-orm/pg-core';
...
export const specOverrides = pgTable('spec_overrides', {
  id: uuid('id').primaryKey().defaultRandom(),
  ruleId: uuid('rule_id').notNull().references(() => specRules.id),
...
export const complianceReports = pgTable('compliance_reports', {
  ...
  overrideEvents: json('override_events').notNull().$type<Array<{
    ruleId: string;
    reason: string;
  }>>(),
  createdAt: timestamp('created_at').notNull().defaultNow(),
