...
6. [实施路线图](#实施路线图)
7. [架构演进规划](#架构演进规划)
8. [MVP 需求规格（2026 Q1 试点）](#mvp-需求规格2026-q1-试点)
...
> 本节为需求澄清后的落地规格，是上述架构愿景在第一期的具体实现边界。

### 平台定位关系

| 层级 | 平台 | 职责 |
|------|------|------|
| 知识底座 | **知行平台** | 企业知识管理 + Spec 规则中心 + 上下文提供者 |
| 工作流引擎 | **OpenSpec** | 运行在知行之上，负责单次变更的规则定义与执行 |
| AI 编程工具 | **Cursor / Claude Code** | 消费知行提供的上下文，执行具体编码任务 |

知行是 OS 层，OpenSpec 和 AI 编程工具是运行在其上的"进程"。

---

### 目标用户与部署方式

- **用户**：内部研发团队（PM、架构师、开发、运维）
- **部署**：私有化部署
- **规模背景**：约 10 个领域 × 10 个子项目 = 100 个项目，代码隔离、知识孤岛
- **试点团队**：10 人，4-5 个项目（Java 后端 + React/JS 前端）
...
**模块一：三层 Spec 管理中心（Web UI）**

- 知行 Web 平台为 Spec 规则的**唯一事实来源（Source of Truth）**
- 三层结构：
  - **公司级**：由中台部门 + 产研架构组共同定义，管理权限收归架构组
  - **领域级**：由各领域架构师定义，权限与公司级隔离
  - **项目级**：团队成员维护，可显式 override 上层规则
- 继承机制：类似 ESLint extends，上层为默认，项目级可显式声明覆盖（须填写覆盖原因）
- 过渡期策略：当前允许覆盖，后期部分红线规则升级为不可覆盖

**模块二：Spec 分发引擎（双轨制）**

- **本地文件轨**：将规则编译为 `CLAUDE.md` / `AGENTS.md`，自动推送到各 GitLab 项目仓库，供 IDE 和 CI 使用
- **MCP 实时轨**：运行时按需从知行 MCP 服务拉取上下文，支持跨领域知识订阅
- **订阅模型**：项目声明订阅哪些领域（如采购项目订阅财务领域知识），MCP 动态注入，非全量嵌入
- **更新触发**：
  - Spec 规则变更 → 准实时触发同步，更新 Spec 文件
  - 代码层索引 → 定期批量推送

**模块三：认知层 MCP 服务（RAG 知识底座）**

- **数据源**：
  - GitLab 代码仓库（部分全量，Java 后端 + React/JS 前端）
  - 需求文档 + API 文档
  - IM 沟通记录（**延后处理**）
- **核心问答能力**：
  - 代码定位："X 模块的核心逻辑在哪里？" → AI 自主检索，无法定位时向用户提问
  - 变更分析：定位文件后自动拉取 Git 历史并分析
  - 上下文依赖："该模块的输入/输出/上下游依赖是什么？"
  - API 规范查询
- **问答模式**：构建需求→代码分析的澄清模型，支持系统级持续迭代

**模块四：CLI 智能体**

- 开发者统一 AI 编程入口，预装公司认知与规则
- 核心差异于原生 Claude Code：
  - 感知 GitLab 代码历史与内部上下文（认知层接入）
  - 内置三层 Spec 规范，强制合规
  - 统一 MCP 工具入口，零配置开箱即用
- 典型场景：
  - 新成员快速理解项目上下文（问认知层）
  - 基于规范和历史知识实现新增功能
  - 跨领域知识订阅辅助（如后端调用财务领域知识）

**模块五：Skill 体系**

Skill 是一个**容器**，包含以下一种或多种形态：

| 形态 | 说明 |
|------|------|
| MCP Tool | 可调用的接口（如"查询订单状态"、"查 K8s 日志"） |
| Prompt 模板 | 经验证的生成模板（如"生成符合公司规范的单元测试"） |
| OpenSpec change 模板 | 预定义的任务拆解流程 |
| 使用说明 | Skill 的使用场景、输入输出说明 |

- **分发机制**：团队沉淀的优质 Skill 通过知行平台分发至各项目，提升 AI 编程效率
- **与 OpenSpec 关系**：知行为 OpenSpec 提供 Skill 上游供给，OpenSpec 负责具体执行

**模块六：合规追踪（数据层）**

- **CI 静态扫描**：代码提交时检测 Spec 遵守情况（第一期只上报，不拦截）
- **数据上报**：记录哪些项目 override 了哪些规则及原因
- **后期分析方向**：
  - 采纳率低 → 规则本身可能有缺陷，触发规则优化
  - 重要项目覆盖红线规则 → 暴露风险，管理层跟进
...
|----|------|
| 团队规模 | 10 人 |
| 项目数量 | 4-5 个（Java 后端 + React/JS 前端） |
| 起始状态 | 部分项目已有 `CLAUDE.md`/`AGENTS.md`，部分从零开始 |
| 验收标准 | 新功能端到端跑通：CLI 理解项目上下文 → 认知层回答代码问题 → 基于规范实现增量功能 |
...
```
知行平台（Source of Truth）
├── Web 管理平台          ← Spec 规则增删改查（三层权限）
├── Spec 分发引擎
│   ├── 推送 CLAUDE.md/AGENTS.md → GitLab 各项目仓库
│   └── MCP 服务实时注入 → CLI / Cursor / OpenSpec
├── 认知层 MCP 服务
│   ├── 索引 GitLab 代码（定期批量）
│   ├── 索引需求文档 + API 文档
│   └── 对外暴露问答 + 跨领域订阅接口
...
依赖现有基础设施：
  GitLab（代码源）
  + 内部项目管理工具（MCP 接入）
  + K8s 集群 + MCP Server 工具链（已就位）
  + 大模型中转代理（已就位）

上游赋能关系：
  知行 → OpenSpec（规则上下文 + 项目知识 + Skill 供给）
  知行 → Cursor / Claude Code（CLAUDE.md + MCP 上下文注入）
```
...
| 决策项 | 选择 | 说明 |
|--------|------|------|
| Spec Source of Truth | 知行平台数据库 | `CLAUDE.md` 等文件是分发产物，非来源 |
| 分发形态 | 双轨（本地文件 + MCP 实时） | 本地供 IDE/CI，MCP 供 CLI 实时注入 |
| 代码索引更新 | 定期批量 | 代码层变更通过定期推送处理 |
| Spec 变更同步 | 准实时触发 | 规则变更后准实时更新 Spec 文件 |
| 合规拦截策略 | 第一期只上报 | 过渡期积累数据，逐步收紧 |
| 规则继承 | 可覆盖但须显式声明 | 保留审计链路，为后期不可覆盖做准备 |
